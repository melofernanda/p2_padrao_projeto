...
public class ConfiguracoesUsuarioActivity extends AppCompatActivity {
    ...
    //Padrão Comportamental (OBSERVER): Definir uma dependência entre objetos, de maneira que quando um objeto muda de estado todos os seus objetos dependentes(exemplo: pedido de produto) são notificados e atualizados automaticamente.
    //onDataChange: Este método é chamado quando os dados no usuarioRef são alterados.
    //O padrão Observer é utilizado no código para realizar ações em resposta a mudanças nos dados (Firebase)...O addListenerForSingleValueEvent é um método do fireBase que registra um observer para uma única alteração de dados
    //Isso significa que o método onDataChange será chamado uma vez quando os dados em usuarioRef forem alterados.
    //O padrão Observer está monitorando (OBSERVAR) mudanças nos dados do Firebase. Ou seja, quando os dados mudam, o método onDataChange é chamado automaticamente, permitindo que a activity ConfiguracoesUsuarioActivity responda e
    //atualize a interface do usuário com os dados mais recentes do usuário.
    //Dessa forma, a aplicação do padrão Observer, nesse contexto específico, envolve a reação a eventos assíncronos (mudanças nos dados do Firebase) por meio dos métodos onDataChange e onCancelled.
